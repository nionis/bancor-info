          ...
          <OverviewDashboard mx="auto" px={[0, 3]}>
              ...
              <FourByFour
                ...
                bottomLeft={
                  <Text fontSize={24} lineHeight={1} fontWeight={500}>
                    {currencyUnit === 'USD'
                      ? '$' + formattedNum(parseFloat(globalData.volume24hrUSDBInUnit), true)
                      : 'Ξ ' + formattedNum(parseFloat(globalData.volume24hrETHInUnit))}
                    {currencyUnit !== 'USD' ? <SmallText> ETH</SmallText> : ''}
                  </Text>
                }
                bottomRight={
                  globalData.volumeChange24hr && isFinite(globalData.volumeChange24hr) ? (
                    <div>
                      {currencyUnit === 'USD'
                        ? getPercentSign(globalData.volumeChange24hr)
                        : getPercentSign(globalData.volumeChange24hr)}
                    </div>
                  ) : (
                    ''
            ...
            <TopPanel rounded color="white" p={24} style={{ gridArea: 'liquidity' }}>
              <FourByFour
                ...
                bottomLeft={
                  <Text fontSize={24} lineHeight={1} fontWeight={500}>
                    {globalData.liquidityETHInUnit
                      ? currencyUnit !== 'USD'
                        ? 'Ξ ' + formattedNum(parseFloat(globalData.liquidityETHInUnit))
                        : '$' + formattedNum(parseFloat(globalData.liquidityUSDBInUnit), true)
                      : '-'}
                    {currencyUnit === 'USD' ? '' : <SmallText> ETH</SmallText>}
                  </Text>
                }
                bottomRight={
                  globalData.liquidityChange24hr ? (
                    <div>
                      {currencyUnit === 'USD'
                        ? getPercentSign(globalData.liquidityChange24hr)
                        : getPercentSign(globalData.liquidityChange24hr)}
                    </div>
                  ) : (
                    ''
            ...
            <TopPanel rounded bg="white" color="white" style={{ gridArea: 'shares' }} p={24}>
              <FourByFour
                ...
                bottomLeft={
                  <Text fontSize={24} lineHeight={1} fontWeight={500}>
                    {formattedNum(globalData.txs)}
                  </Text>
                }
                bottomRight={globalData.txsChange24hr ? <div>{getPercentSign(globalData.txsChange24hr)}</div> : ''}
              />
            </TopPanel>
